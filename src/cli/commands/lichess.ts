  ...
  try {
    if (!username || username.trim() === '') {
      handleCliError(new ValidationError('Username is required', [
        'Provide a Lichess username',
        'Usage: chess-game-importer lichess <username> [start-date] [end-date]',
      ]));
      process.exit(1);
    }
    ...
    try {
      ...
      if (!isValidDateRange(startDate, endDate)) {
        handleCliError(new ValidationError('Start date must be before or equal to end date', [
          ...
          'Example: chess-game-importer lichess myuser 2024-01-01 2024-12-31',
        ]));
        process.exit(1);
      }
      ...
      if (error instanceof Error) {
        handleCliError(new ImportError(error.message, 'LICHESS_API_ERROR', [
          'Check that Lichess username is correct',
          'Verify your network connection',
          'Lichess may be rate limiting - try again later',
        ], { operation: 'import-lichess', target: username }));
      }
      process.exit(1);
    } finally {
      stockfish.terminate();
